...
import { _decorator, Component, Vec2, Node, RenderComponent, gfx } from 'cc';
...
@ccclass('BezierAssembler')
export class BezierAssembler extends PageEffectAssemblerBase {
    ...
    private _getCtrlPosByAngle(width: number): {startPos: Vec2, endPos: Vec2,
        ctrlPos1: Vec2, ctrlPos2: Vec2} {
    ...
    // 修正宽度
    private _fixWidth(lastBezierPos: Vec2, nextBezierPos: Vec2, 
        width: number, realWidth: number, isTail: boolean) {
        ...
        if(isTail) {
            ...
            nextBezierPos = lastBezierPos.add(direction.multiplyScalar(gapWidth))
    ...
    // 贝塞尔曲线公式
    private _getBezierPos(t: number, startPos: Vec2, endPos: Vec2,
        ctrlPos1: Vec2, ctrlPos2: Vec2): Vec2 {
